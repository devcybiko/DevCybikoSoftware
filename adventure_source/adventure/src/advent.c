...
process_message(struct Message *message) {}

main(int argc, char **argv)
{
	int	rflag;		/* user restore request option	*/
    DBG0(">main %s", __FILE__);
...
scanint(int *pi, char *str)
{
    ...
    DBG0(">scanint %s", __FILE__);
	while (*str) {
        x = sscanf(str, "%d,", pi);
        DBG0("%d", pi[0]);
        pi++;
		if  (x < 1) bug(41);	/* failed before EOS	*/
...
saveadv(void)
{
	...
	fwrite( &iclock, sizeof(int), 1, savefd);
	fwrite( &iclock2, sizeof(int), 1, savefd);
...
restore(void)
{
	...
	fread( &iclock, sizeof(int), 1, restfd);
	fread( &iclock2, sizeof(int), 1, restfd);
...
int sscanf(char *in, char *fmt, ...)
{
        ...
        else if (SAME("%19s %19s", fmt))
        {
                strcpy((char *)x[1], "");
                strcpy((char *)x[2], "");
                strcpy(scanf_buffer, in);
                for(t=scanf_buffer; *t; t++) if (!iswhite(*t)) break;
