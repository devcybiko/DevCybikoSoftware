...
int goback(void)
{
	...
	copytrv(itravel, strav);
	for (kk = 0; itravel[kk].tdest != -1; ++kk) {
		if (!itravel[kk].tcond && itravel[kk].tdest == want) {
			motion = itravel[kk].tverb;
			dotrav();
			return;
		}
		if (!itravel[kk].tcond) {
			k2 = kk;
			temp = itravel[kk].tdest;
			gettrav(temp);
			if (forced(temp) && itravel[0].tdest == want)
				k2 = temp;
			copytrv(strav, itravel);
		}
	}
	if (k2) {
		motion = itravel[k2].tverb;
		dotrav();
	}
	else
		rspeak(140);
...
/*
	Routine to copy a itravel array
*/
int copytrv(struct trav *trav1, struct trav *trav2)
{
	int	i;
...
int dotrav(void)
{
	...
	for (kk = 0; itravel[kk].tdest>=0 && !mvflag; ++kk) {
		rdest = itravel[kk].tdest;
		rverb = itravel[kk].tverb;
		rcond = itravel[kk].tcond;
		robject = rcond%100;
